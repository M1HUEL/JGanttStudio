		...
		for (TaskLink link : project.links()) {
			...
			LocalDate bound = switch (link.type()) {
				case FINISH_TO_START ->
					predecessor.range().end().plusDays(link.lag().days());
				case START_TO_START ->
					predecessor.range().start().plusDays(link.lag().days());
				case FINISH_TO_FINISH ->
					predecessor.range().end().plusDays(link.lag().days()).minusDays(duration - 1);
				case START_TO_FINISH ->
					predecessor.range().start().plusDays(link.lag().days()).minusDays(duration - 1);
			};
